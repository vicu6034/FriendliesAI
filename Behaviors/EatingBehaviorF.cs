...
{
    public class EatingBehaviorF : IBehaviour
    {
        ...
        public Vector3 LastKnownFoodPosition
        {
            get
            {
                ...
                {
                    return m_aiBase.NView.GetZDO()
                        .GetVec3(Constants.Z_SavedFoodPosition, m_aiBase.Character.transform.position);
                }
        ...
        private StateMachine<string, string>.TriggerWithParameters<IEnumerable<ItemDrop.ItemData>, string, string>
            LookForItemTrigger;
        ...
        public void Configure(MobAIBase aiBase, StateMachine<string, string> brain, string parentState)
        {
            ...
            LookForItemTrigger =
                brain.SetTriggerParameters<IEnumerable<ItemDrop.ItemData>, string, string>(Trigger.ItemFound);
            ...
            brain.Configure(State.Hungry)
                ...
                .OnExit(t => {}); //Debug.LogWarning("Exiting EatingBehaviour");
            ...
            brain.Configure(State.SearchForFood)
                ...
                {
                    m_foodsearchtimer = 0f;
                    brain.Fire(LookForItemTrigger,
                        (aiBase.Instance as MonsterAI).m_consumeItems.Select(i => i.m_itemData), State.HaveFoodItem,
                        State.HaveNoFoodItem);
                });
            ...
            brain.Configure(State.HaveFoodItem)
                ...
                .OnEntry(t =>
                {
                    ...
                    (aiBase.Instance as MonsterAI).m_onConsumedItem((aiBase.Instance as MonsterAI).m_consumeItems
                        .FirstOrDefault());
                    (aiBase.Instance.GetComponent<Character>() as Humanoid).m_consumeItemEffects.Create(
                        aiBase.Instance.transform.position, Quaternion.identity);
                    var animator =
                        aiBase.Instance.GetType().GetField("m_animator", BindingFlags.Instance | BindingFlags.NonPublic)
                            .GetValue(aiBase.Instance) as ZSyncAnimation;
                ...
                .OnExit(t => {}); //Debug.LogWarning($"Exiting HaveFoodItem, Trigger:{t.Trigger}");

            brain.Configure(State.HaveNoFoodItem)
                .SubstateOf(State.Hungry)
                .PermitIf(Trigger.ItemNotFound, State.Hungry)
                .OnEntry(t => { brain.Fire(Trigger.ItemNotFound); });
