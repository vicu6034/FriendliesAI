...
namespace FriendliesAI
{
    public static class GrootConfig
    {
        ...
        public static void Init(ConfigFile Config)
        {
            ...
            GrootConfig.TamingTime = Config.Bind<int>("General", "Brute_TamingTime", 5, "Total time it takes to tame Groot");
            GrootConfig.AssignmentSearchRadius = Config.Bind<int>("General", "Groot_AssignmentSearchRadius", 10, "Radius to search for new assignments within");
            GrootConfig.ItemSearchRadius = Config.Bind<int>("General", "Groot_ItemSearchRadius", 10, "Radius to search for items on the ground");
            GrootConfig.ContainerSearchRadius = Config.Bind<int>("General", "Groot_ContainerSearchRadius", 10, "Radius to search for containers");
            GrootConfig.MaxContainersInMemory = Config.Bind<int>("General", "Groot_MaxContainersInMemory", 3, "How many containers Groot should remember contents from");
            GrootConfig.TimeLimitOnAssignment = Config.Bind<int>("General", "Groot_TimeLimitOnAssignment", 30, "How long before moving on to next assignment");
            GrootConfig.IncludedContainersList = Config.Bind<string>("General", "Groot_IncludedContainersList", "piece_chest_wood", "Comma separated list of container piece names to be searchable by Groot");
